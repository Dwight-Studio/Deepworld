...
public class ContainerWoodenLathe extends Container {
    public static ContainerWoodenLathe createContainerServerSide(int windowID, PlayerInventory playerInventory,
                                                                      WoodenLatheZoneContents inputZoneContents,
                                                                      WoodenLatheZoneContents outputZoneContents,
                                                                      WoodenLatheStateData woodenLatheStateData) {
        return new ContainerWoodenLathe(windowID, playerInventory, inputZoneContents, outputZoneContents, woodenLatheStateData);
    }

    public static ContainerWoodenLathe createContainerClientSide(int windowID, PlayerInventory playerInventory, net.minecraft.network.PacketBuffer extraData) {
        ...
        WoodenLatheZoneContents inputZoneContents = WoodenLatheZoneContents.createForClientSideContainer(INPUT_SLOTS_COUNT);
        WoodenLatheZoneContents outputZoneContents = WoodenLatheZoneContents.createForClientSideContainer(OUTPUT_SLOTS_COUNT);
        WoodenLatheStateData woodenLatheStateData = new WoodenLatheStateData();
    ...
    //  37 = input slot (woodenLatheStateData 0)
    //  38 = output slot (woodenLatheStateData 1)
    ...
    public static final int INPUT_SLOTS_COUNT = TileEntityWoodenLathe.INPUT_SLOTS_COUNT;
    public static final int OUTPUT_SLOTS_COUNT = TileEntityWoodenLathe.OUTPUT_SLOTS_COUNT;
    public static final int WOODEN_LATHE_SLOTS_COUNT = INPUT_SLOTS_COUNT + OUTPUT_SLOTS_COUNT;
    ...
    public ContainerWoodenLathe(int windowID, PlayerInventory invPlayer,
                                     WoodenLatheZoneContents inputZoneContents,
                                     WoodenLatheZoneContents outputZoneContents,
                                     WoodenLatheStateData woodenLatheStateData) {
        super(DeepworldContainers.WOODEN_LATHE_CONTAINER, windowID);
        if (DeepworldContainers.WOODEN_LATHE_CONTAINER == null)
            throw new IllegalStateException("Must initialise containerTypeContainerWoodenLathe before constructing a ContainerWoodenLathe!");
        this.inputZoneContents = inputZoneContents;
        this.outputZoneContents = outputZoneContents;
        this.woodenLatheStateData = woodenLatheStateData;
        this.world = invPlayer.player.world;

        trackIntArray(woodenLatheStateData);    // tell vanilla to keep the woodenLatheStateData synchronised between client and server Containers
        ...
        for (int y = 0; y < INPUT_SLOTS_COUNT; y++) {
            int slotNumber = y;
            addSlot(new ContainerWoodenLathe.SlotProcessableInput(inputZoneContents, slotNumber, INPUT_SLOT_XPOS, INPUT_SLOT_YPOS));
        }
        ...
        for (int y = 0; y < OUTPUT_SLOTS_COUNT; y++) {
            int slotNumber = y;
            addSlot(new ContainerWoodenLathe.SlotOutput(outputZoneContents, slotNumber, OUTPUT_SLOT_XPOS, OUTPUT_SLOT_YPOS));
        }
    }
    ...
    public ItemStack transferStackInSlot(PlayerEntity player, int sourceSlotIndex)
    {
        ...
        ContainerWoodenLathe.SlotZone sourceZone = ContainerWoodenLathe.SlotZone.getZoneFromIndex(sourceSlotIndex);

        switch (sourceZone) {
            case OUTPUT_ZONE: // taking out of the output zone - try the hotbar first, then main inventory.  fill from the end.
                successfulTransfer = mergeInto(ContainerWoodenLathe.SlotZone.PLAYER_HOTBAR, sourceItemStack, true);
                if (!successfulTransfer) {
                    successfulTransfer = mergeInto(ContainerWoodenLathe.SlotZone.PLAYER_MAIN_INVENTORY, sourceItemStack, true);
                }
            ...
            case INPUT_ZONE: // taking out of input zone - try player main inv first, then hotbar.  fill from the start
                successfulTransfer = mergeInto(ContainerWoodenLathe.SlotZone.PLAYER_MAIN_INVENTORY, sourceItemStack, false);
                if (!successfulTransfer) {
                    successfulTransfer = mergeInto(ContainerWoodenLathe.SlotZone.PLAYER_HOTBAR, sourceItemStack, false);
                }
                break;

            case PLAYER_HOTBAR:
            case PLAYER_MAIN_INVENTORY: // taking out of inventory - find the appropriate furnace zone
                if (TileEntityWoodenLathe.getMatchingRecipeForInput(world, sourceItemStack) != null) { // processable -> add to input
                    successfulTransfer = mergeInto(ContainerWoodenLathe.SlotZone.INPUT_ZONE, sourceItemStack, false);
                }
                if (!successfulTransfer) {  // didn't fit into furnace; try player main inventory or hotbar
                    if (sourceZone == ContainerWoodenLathe.SlotZone.PLAYER_HOTBAR) { // main inventory
                        successfulTransfer = mergeInto(ContainerWoodenLathe.SlotZone.PLAYER_MAIN_INVENTORY, sourceItemStack, false);
                    } else {
                        successfulTransfer = mergeInto(ContainerWoodenLathe.SlotZone.PLAYER_HOTBAR, sourceItemStack, false);
                    }
     ...
     */
    private boolean mergeInto(ContainerWoodenLathe.SlotZone destinationZone, ItemStack sourceItemStack, boolean fillFromEnd) {
    ...
    public double fractionOfProcessTimeComplete() {
        if (woodenLatheStateData.processTimeForCompletion == 0) return 0;
        double fraction = woodenLatheStateData.processTimeElapsed / (double)woodenLatheStateData.processTimeForCompletion;
        return MathHelper.clamp(fraction, 0.0, 1.0);
    }

    @Override
    public boolean enchantItem(PlayerEntity playerIn, int id) {
        woodenLatheStateData.inertiaTimeRemaining += 10;
        woodenLatheStateData.inertiaTimeRemaining = Math.min(woodenLatheStateData.inertiaTimeRemaining, woodenLatheStateData.inertiaTimeInitialValue);
        woodenLatheStateData.inertiaTimeInitialValue = 100;
        return true;
    }
    ...
    public static class SlotProcessableInput extends Slot {
        ...
        public boolean isItemValid(ItemStack stack) {
            return TileEntityWoodenLathe.isItemValidForInputSlot(stack);
        }
    }
    ...
    public static class SlotOutput extends Slot {
        ...
        @Override
        public boolean isItemValid(ItemStack stack) {
            return TileEntityWoodenLathe.isItemValidForOutputSlot(stack);
        }
    }

    private WoodenLatheZoneContents inputZoneContents;
    private WoodenLatheZoneContents outputZoneContents;
    private WoodenLatheStateData woodenLatheStateData;
    ...
    private enum SlotZone {
        ...
        public static ContainerWoodenLathe.SlotZone getZoneFromIndex(int slotIndex) {
            for (ContainerWoodenLathe.SlotZone slotZone : ContainerWoodenLathe.SlotZone.values()) {
