...
public class WoodenPressZoneContents implements IInventory {

    /**
     * Use this constructor to create a WoodenPressZoneContents which is linked to its parent TileEntity.
     ...
     */
    public static WoodenPressZoneContents createForTileEntity(int size,
                                                          Predicate<PlayerEntity> canPlayerAccessInventoryLambda,
                                                          Notify markDirtyNotificationLambda) {
        return new WoodenPressZoneContents(size, canPlayerAccessInventoryLambda, markDirtyNotificationLambda);
    }

    /**
     * Use this constructor to create a WoodenPressZoneContents which is not linked to any parent TileEntity; i.e.
     ...
     * @return the new ChestContents
     */
    public static WoodenPressZoneContents createForClientSideContainer(int size) {
        return new WoodenPressZoneContents(size);
    }
    ...
    public CompoundNBT serializeNBT()  {
        return WoodenPressContents.serializeNBT();
    }
    ...
    public void deserializeNBT(CompoundNBT nbt)   {
        WoodenPressContents.deserializeNBT(nbt);
    }
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        return WoodenPressContents.isItemValid(index, stack);
    }
    ...
    public int getSizeInventory() {
        return WoodenPressContents.getSlots();
    }

    @Override
    public boolean isEmpty() {
        for (int i = 0; i < WoodenPressContents.getSlots(); ++i) {
            if (!WoodenPressContents.getStackInSlot(i).isEmpty()) return false;
        }
        return true;
    }

    public ItemStack getStackInSlot(int index) {
        return WoodenPressContents.getStackInSlot(index);
    }

    @Override
    public ItemStack decrStackSize(int index, int count) {
        if (count < 0) throw new IllegalArgumentException("count should be >= 0:" + count);
        return WoodenPressContents.extractItem(index, count, false);
    }

    @Override
    public ItemStack removeStackFromSlot(int index) {
        int maxPossibleItemStackSize = WoodenPressContents.getSlotLimit(index);
        return WoodenPressContents.extractItem(index, maxPossibleItemStackSize, false);
    }

    @Override
    public void setInventorySlotContents(int index, ItemStack stack) {
        WoodenPressContents.setStackInSlot(index, stack);
    }

    @Override
    public void clear() {
        for (int i = 0; i < WoodenPressContents.getSlots(); ++i) {
            WoodenPressContents.setStackInSlot(i, ItemStack.EMPTY);
        }
    }
    ...
    public ItemStack increaseStackSize(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = WoodenPressContents.insertItem(index, itemStackToInsert, false);
    ...
    public boolean doesItemStackFit(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = WoodenPressContents.insertItem(index, itemStackToInsert, true);
        return leftoverItemStack.isEmpty();
    }

    // ---------

    private WoodenPressZoneContents(int size) {
        this.WoodenPressContents = new ItemStackHandler(size);
    }

    private WoodenPressZoneContents(int size, Predicate<PlayerEntity> canPlayerAccessInventoryLambda, Notify markDirtyNotificationLambda) {
        this.WoodenPressContents = new ItemStackHandler(size);
        this.canPlayerAccessInventoryLambda = canPlayerAccessInventoryLambda;
        this.markDirtyNotificationLambda = markDirtyNotificationLambda;
    }
    ...
    private final ItemStackHandler WoodenPressContents;
}
